...
public class Bot : MonoBehaviour
{
  ...
  void Update()
  {
    ...
    if (dodge)
    {
      ...
      float closestDistance = sizeOfArena;
      for (int i = 0; i < projectiles.Length; i++)
      {
        if (closestProjectile == null)
        {
          closestDistance = Vector2.Distance(projectiles[i].transform.position, this.transform.position);
      ...
      closestDistance = Mathf.Min(closestDistance / sizeOfArena, 1.0f);
    ...
    if (fight)
    {
      ...
      GameObject closestProjectile = null;
      float closestDistance = sizeOfArena;
      Debug.Log("projectile number:" + projectiles.Length);
